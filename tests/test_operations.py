...
from scripts.operations import cmd_reset_quotas, cmd_auto_update, cmd_usage_report
...
def test_auto_update_applies_feed(tmp_path: Path) -> None:
    ...
    feed_file.write_text(json.dumps({"models": [
        {"model_id": "x/y", "provider": "x", "display_name": "Y",
         "context_window": 1000, "input_price": 0.0, "output_price": 0.0,
         "is_free": True, "tier_rank": 1, "strength_tags": [],
         "weakness_tags": [], "best_for": [], "performance_score": 50.0},
        {"model_id": "new/z", "provider": "n", "display_name": "Z",
         "context_window": 1000, "input_price": 0.0, "output_price": 0.0,
         "is_free": True, "tier_rank": 2, "strength_tags": [],
...
def test_usage_report_handles_malformed_lines(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    log = tmp_path / "router.jsonl"
    log.write_text("not json\n" + json.dumps({"model_used": "x", "total_tokens": 5, "duration_s": 0.1, "fallback_used": False, "preserve_paid_quota": True, "confidence": 0.5, "error": None}) + "\n")
    monkeypatch.chdir(tmp_path)
    rc = cmd_usage_report()
    assert rc == 0
