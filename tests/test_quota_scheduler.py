...
class TestMaybeResetDue:
    ...
    def test_empty_schedule_is_manual_only(self, qm):
        ...
        _seed_quota(store, "model-c", remaining=1, last_reset_iso=yesterday, schedule="")
        n = q.maybe_reset_due()
        assert n == 0
    ...
    def test_idempotent_when_nothing_due(self, qm):
        q, store = qm
        _seed_quota(store, "model-x", remaining=5, last_reset_iso=datetime.now(timezone.utc).isoformat())
        assert q.maybe_reset_due() == 0
        assert q.maybe_reset_due() == 0  # calling twice doesn't change anything
