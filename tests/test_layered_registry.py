...
def curated_path(tmp_path: Path) -> Path:
    ...
    p.write_text(json.dumps({
        "version": "2026-06-10",
        "models": [
            {"model_id": "c/a", "provider": "c", "display_name": "Curated A",
             "context_window": 1000, "input_price": 0.0, "output_price": 0.0,
             "is_free": True, "tier_rank": 1, "strength_tags": ["deep_reasoning"],
...
def discovered_path(tmp_path: Path) -> Path:
    ...
    p.write_text(json.dumps({
        "version": "2026-06-10",
        "models": [
            {"model_id": "d/x", "provider": "d", "display_name": "Disc X",
             "context_window": 1000, "input_price": 0.0, "output_price": 0.0,
             "is_free": True, "tier_rank": 20, "strength_tags": ["coding_sota"],
             "weakness_tags": [], "best_for": [], "performance_score": 80.0},
            {"model_id": "d/y", "provider": "d", "display_name": "Disc Y",
             "context_window": 2000, "input_price": 0.0, "output_price": 0.0,
             "is_free": True, "tier_rank": 30, "strength_tags": ["vision_master"],
             "weakness_tags": [], "best_for": [], "performance_score": 85.0},
        ],
    }))
    return p
...
def test_layered_curated_wins_on_conflict(curated_path, tmp_path) -> None:
    ...
    dup.write_text(json.dumps({
        "models": [
            {"model_id": "c/a", "provider": "d", "display_name": "DUPLICATE",
             "context_window": 999, "input_price": 0.0, "output_price": 0.0,
             "is_free": True, "tier_rank": 99, "strength_tags": ["junk"],
