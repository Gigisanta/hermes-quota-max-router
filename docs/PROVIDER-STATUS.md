...
## Cuentas de MaatWork, ordenadas por evidencia editorial

Hay **cuatro altas con credencial guardada**: tres claves en el almacén privado
del servicio y una clave de Final Router aislada para evaluación. El orden de
esta tabla refleja acceso y pruebas editoriales observados, **no** una medición
comparable de inteligencia ni una ruta productiva. Los modelos de una misma
cuenta comparten sus límites según las reglas del operador; no se suman como
proveedores independientes. RPM/RPD son solicitudes por minuto/día y TPM/TPD
tokens por minuto/día; RPH/TPH indican la ventana horaria.
Las cuotas numéricas son las observadas en la cuenta el **25 de septiembre de
2026**, no capacidad garantizada ni límites públicos inmutables. Para Gemini,
Google remite a [AI Studio](https://ai.google.dev/gemini-api/docs/rate-limits)
para consultar el límite vigente del proyecto.

| Orden | Cuenta registrada y estado | Modelos gratuitos confirmados para esta cuenta | Cuota gratuita observada | Evidencia y uso actual |
| ---: | --- | --- | --- | --- |
| 1 | **Gemini Free**, clave dedicada restringida a Gemini API; candidato editorial | [`gemini-3.5-flash-lite`](https://ai.google.dev/gemini-api/docs/pricing) a $0 en Free, con generación real | **15 RPM, 500 RPD, 250.000 TPM** para el modelo, por proyecto; TPD no acreditado | 60/60 revisiones sintéticas de Journal y una repetición de formato de autor válida; no aprobado por carga ni en producción. Otros modelos listados fallaron el smoke o quedaron indisponibles. |
| 2 | **SimpleLLM Free**, clave dedicada; candidato editorial | [`gemma-4-E4B`](https://simplellm.eu/docs/models.html) a **0 SC** de entrada y salida, disponible; los demás modelos de precio cero publicados figuraban `planned` | [**100 RPH, 1.000 RPD, 50.000 TPH, 500.000 TPD**](https://simplellm.eu/docs/limits.html) para modelos gratuitos, por cuenta; API informó 1.000 RPM generales, pero la [página comercial](https://simplellm.eu/) anuncia 10 RPM | Los 60 casos sintéticos de revisión de Journal obtuvieron veredicto correcto tras ampliar a 1.024 tokens tres respuestas incompletas; una repetición de formato de autor válida. Falta calidad editorial completa. |
| 3 | **Groq Free**, clave del titular guardada; candidato editorial | `openai/gpt-oss-120b` con generación real; `openai/gpt-oss-20b` y `qwen/qwen3.8-27b` (Preview) listados en la cuenta, sin smoke propio | La consola mostró [**30 RPM, 1.000 RPD, 8.000 TPM, 200.000 TPD**](https://console.groq.com/docs/rate-limits) para esos modelos, por organización y modelo | `gpt-oss-120b` pasó un smoke y 2/2 casos iniciales de revisión de Journal; falta muestra comparable y prueba de autor. |
| 4 | **Final Router Free**, clave sólo de evaluación; **bloqueado para contenido editorial** | [`openai/gpt-5-mini` y `deepseek/deepseek-v4-flash`](https://finalrouter.com/free-models) figuran gratis dentro del cupo; GPT-5 mini pasó un smoke y DeepSeek sólo recibió solicitudes sintéticas para probar el guardrail | [**20 RPM, 25 RPD**](https://finalrouter.com/pricing) publicados; presupuestar los 25 por cuenta sin multiplicar por clave o modelo; respuesta hasta 8.000 tokens, sin crédito pago | El guardrail asignado a la clave permitió DeepSeek pese a la lista cerrada para GPT-5 mini. No se incorporó a `.env` ni al catálogo productivo. |

**Ninguno de los cuatro suma todavía a la reserva de producción.** La tabla
detallada siguiente registra fuentes, pruebas negativas, condiciones de datos y
lo que falta para admitir cada modelo. Cloudflare, SiliconFlow y Vikasit siguen
sin una cuenta MaatWork apta con clave API verificada; reconocer una identidad en
una consola o tener otra cuenta personal no equivale a esa alta.
...
| Cloudflare Workers AI Free | [Workers Free incluye 10.000 Neurons diarios](https://developers.cloudflare.com/workers-ai/platform/pricing/), reinicia a las 00:00 UTC y corta al agotar ese cupo. Es una capacidad gratuita con límite duro, aunque los modelos tengan tarifa unitaria positiva fuera de ese cupo; no se usa un plan pago ni créditos. La misma tabla identifica GLM 5.3 Flash y otros modelos como exclusivos de Workers Paid o créditos prepagos; el catálogo del router los rechaza. El [changelog oficial](https://developers.cloudflare.com/changelog/post/2026-07-28-models-require-workers-paid/) identifica `@cf/zai-org/glm-4.7-flash` y `@cf/google/gemma-4-26b-a4b-it` como disponibles en Workers Free. Cloudflare afirma que [no usa los datos enviados a Workers AI para entrenar sus servicios sin consentimiento](https://developers.cloudflare.com/workers-ai/platform/data-usage/), sujeto también a las licencias de cada modelo. | Acceso de MaatWork pendiente de consentimiento de términos, token acotado a Workers AI, tasa real de Neurons, smoke gratuito y calidad editorial de esos modelos. |
...
| SimpleLLM Free | MaatWork completó el alta con contraseña propia. El 25 de septiembre, el [catálogo público oficial](https://simplellm.eu/docs/models.html) y su API marcaron `gemma-4-E4B` disponible, con entrada y salida a **0 SC** y contexto de 131.072 tokens. Llama 8B, Mistral 7B y Kode 14B aparecieron a 0 SC pero `planned`, y no se cuentan aunque la página comercial anuncie algunos. Hay una clave dedicada guardada sólo en el almacén local del router. `GET /v1/rate-limit` devolvió 1.000 RPM generales, 100 solicitudes/hora y 1.000/día, 50.000 tokens/hora y 500.000/día para modelos gratuitos. La [página comercial](https://simplellm.eu/) anuncia 10 RPM: hasta aclarar la diferencia, usar la menor cifra como tope minuto. Tres llamadas sintéticas a `gemma-4-E4B` devolvieron HTTP 200; una con 120 tokens de salida no produjo texto, y otra con 512 produjo un resumen fiel. `GET /v1/usage` y el uso por clave registraron 3 solicitudes, **0 SC gastados** y saldo 0 SC. El router reserva las ventanas horarias y limita a una solicitud SimpleLLM simultánea. [Documentación de cuotas](https://simplellm.eu/docs/limits.html). | Falta comprobar vigencia y reinicio real de ventanas, evaluar los casos editoriales de cada carga y verificar que el modelo pequeño puede servir como autor o revisor. El smoke sintético y el saldo 0 SC no acreditan calidad ni el piloto de siete días. Sigue deshabilitado para producción y no suma a los cuatro proveedores aptos. |
...
de costo facturable cero con cupo estable y corte duro. [Freebuff](https://freebuff.com/terms-of-service)
